...
def get_post_db():
    if get_post_db.value is None:
        get_post_db.value = sqlite3.connect(
            os.environ.get("POST_DB_PATH")
            or (os.path.join(get_data_dir(), "posts.db")),
...
def get_feed(feed_id):
    ...
    record = cur.fetchone()
    return (
        {"id": record[0], "last_fetch_date": record[1]} if record is not None else None
    )
...
def add_post(feed_id, post_url, err=None):
    ...
    cur.execute("INSERT OR IGNORE INTO feeds(id) VALUES (:feed)", {"feed": feed_id})
